...
(function () {
  ...
  function lineChartLabels(container, opts) {
    ...
    for (const tv of niceTicks(yMin, yMax, 5)) {
      ...
      el("line", { x1: m.left, x2: W - m.right, y1: Y(tv), y2: Y(tv), stroke: "#d5c9ab", "stroke-width": 1 }, svg);
      const t = el("text", { x: m.left - 8, y: Y(tv) + 4, "text-anchor": "end", fill: "#857a5f", "font-size": 11 }, svg);
      t.textContent = fmt(tv);
    }
    el("line", { x1: m.left, x2: W - m.right, y1: m.top + ih, y2: m.top + ih, stroke: "#a99c7d", "stroke-width": 1 }, svg);
    // x labels (thin out)
    const every = Math.ceil(nX / Math.floor(iw / 70));
    opts.xLabels.forEach((lb, i) => {
      if (i % every !== 0 && i !== nX - 1) return;
      const t = el("text", { x: X(i), y: H - 8, "text-anchor": "middle", fill: "#857a5f", "font-size": 11 }, svg);
      t.textContent = lb;
    });
    ...
    for (const s of series) {
      ...
      el("circle", { cx: X(last.x), cy: Y(last.y), r: 4, fill: s.color, stroke: "#f4eddc", "stroke-width": 2 }, svg);
    }
    ...
    if (series.length <= 4) {
      series.forEach(s => {
        ...
        const last = pts[pts.length - 1];
        const t = el("text", { x: Math.min(X(last.x) + 8, W - 2), y: Y(last.y) + 4, fill: "#453f30", "font-size": 11 }, svg);
  ...
  function lineChartLinear(container, opts) {
    ...
    for (const tx of niceTicks(xMin, xMax, Math.min(10, Math.floor(iw / 80)))) {
      ...
      const t = el("text", { x: X(tx), y: H - 8, "text-anchor": "middle", fill: "#857a5f", "font-size": 11 }, svg);
    ...
    if (badges.length) {
      ...
      for (const b of badges) {
        ...
        el("circle", { cx: b.x + 12, cy: by, r: 7.5, fill: b.color, stroke: "#f4eddc", "stroke-width": 1.5 }, svg);
  ...
  function barChart(container, opts) {
    ...
    for (const tv of niceTicks(0, yMax, 4)) {
      if (tv > yMax) continue;
      el("line", { x1: m.left, x2: W - m.right, y1: Y(tv), y2: Y(tv), stroke: "#d5c9ab", "stroke-width": 1 }, svg);
      const t = el("text", { x: m.left - 7, y: Y(tv) + 4, "text-anchor": "end", fill: "#857a5f", "font-size": 11 }, svg);
